...
<html>
  <head>
	...
	<link href="/supermall/style/regist.css" rel="stylesheet" type="text/css" />
	...
	<script type="text/javascript" language="javascript">
		...
		$(document).ready(function(){
			...
			function checkUserName(){
				...
				var reg = /^\w{4,20}$/;
				if(!reg.test(name)){
					$("#username_error").html("4-20个字符，可由字母、数字、“_”组成");
					$("#username_error").css("color","#ff0000");
					return false;
				}
				if(name.length >= 4 && name.length <= 20){
					...
					//使用同步请求才能即时改变b的值
					$.ajax({
						url: "${pageContext.request.contextPath}/servlet/UserServlet?flag=nameIsExist",
						type: "POST",
						async: false,
						data: {"name":name},
						dataType: "json",
...
//  							var isExist = data.isExist;
							if (!data) {
								$("#username_succeed").html("用户名可用");
								$("#username_succeed").css("color","#00ff00");
 								b = true;
							} else {
								$("#username_error").html("用户名已经存在，不能使用该用户名注册");
								$("#username_error").css("color","#ff0000");
								b = false;
							}
						}
			...
			function checkPwd(){
				...
				if(pwd.length == 0){
					...
					$("#pwd_error").css("color","#ff0000");
					return false;
				}
				var reg = /^\w{6,20}$/;
				if(!reg.test(pwd)){
					$("#pwd_error").html("6-20个字符，可由字母、数字、“_”组成");
					$("#pwd_error").css("color","#ff0000");
					return false;
				} else {
					$("#pwd_succeed").html("格式正确");
					$("#pwd_succeed").css("color","#00ff00");
					return true;
				}
				return false;
			}
			...
			function checkPwd2(){
				...
				var pwd2 = $("#pwd2").val();
				if(pwd2.length == 0){
					$("#pwd2_error").html("请输入确认密码");
					$("#pwd2_error").css("color","#ff0000");
					return false;
				}
				if(pwd == pwd2){
					$("#pwd2_succeed").html("密码一致");
					$("#pwd2_succeed").css("color","#00ff00");
					return true;
				} else {
					$("#pwd2_error").html("两次输入的密码不一致");
					$("#pwd2_error").css("color","#ff0000");
					return false;
				}
				return false;
			...
			function checkEmail(){
				...
				if(mail.length == 0){
					...
					$("#mail_error").css("color","#ff0000");
					return false;
				}
				var reg = /^\w{1,}@\w+(\.\w+)+$/;
				if(reg.test(mail)){
					$("#mail_succeed").html("格式正确");
					$("#mail_succeed").css("color","#00ff00");
					return true;
				} else {
					$("#mail_error").html("邮箱格式错误，请输入您的邮箱");
					$("#mail_error").css("color","#ff0000");
					return false;
				}
				return false;
			...
			function checkCode(){
				...
				//使用同步请求才能即时改变b的值
				$.ajax({
					url: "${pageContext.request.contextPath}/servlet/UserServlet?flag=codeIsRight",
					type: "POST",
					async: false,
					data: {"code":code},
					dataType: "json",
			...
			$("#username").focus(function(){
				$("#username_error").html("4-20个字符，可由字母、数字、“_”组成");
				$("#username_error").css("color","#bbb");
			});
			$("#username").blur(checkUserName);
			$("#pwd").focus(function(){
				$("#pwd_error").html("6-20个字符，可由字母、数字、“_”组成");
				$("#pwd_error").css("color","#bbb");
			});
			$("#pwd").blur(checkPwd);
			$("#pwd2").focus(function(){
				$("#pwd2_error").html("请输入确认密码，两次输入的密码必须一致");
				$("#pwd2_error").css("color","#bbb");
			});
			$("#pwd2").blur(checkPwd2);
			$("#mail").focus(function(){
				$("#mail_error").html("请输入您的邮箱");
				$("#mail_error").css("color","#bbb");
			});
			...
			$("#registsubmit").click(function(){
				var b = checkUserName() & checkPwd() & checkPwd2() & checkEmail() & checkCode();
				if(b){
					$("#formpersonal").attr("action","${pageContext.request.contextPath}/servlet/UserServlet?flag=reg");
					$("#formpersonal")[0].submit();
				}
			});
		});
	</script>
...
<div class="l_w" id="regist">
<div class="mt">
    <h2>
        注册新用户</h2>
    <b></b><span>我已经注册，现在就&nbsp;<a href="shop/page/user/login.jsp" class="flk13">登录</a></span>
</div>
<div class="mc">
...
<form id="formpersonal" method="post" onsubmit="return false">
    <div class="form">
        ...
        <div class="item">
            ...
            <input type="button" class="btn-img btn-regist" id="registsubmit" value="同意以下协议，提交" tabindex="8"/>&nbsp;&nbsp;&nbsp;&nbsp;<font color="red" style="font-size: 12px">${requestScope.msg}</font>
        </div>
    </div>
</form>
...
<div id="protocol-con">
    ...
    <p>
        本协议是您与极目商城网站（简称"本站"，网址：www.360buy.com）所有者（以下简称为"极目商城"）之间就极目商城网站服务等相关事宜所订立的契约，请您仔细阅读本注册协议，您点击"同意以下协议，提交"按钮后，本协议即构成对双方有约束力的法律文件。</p>
    <h5> 第1条 本站服务条款的确认和接纳</h5>

    <p>
        <strong>1.1</strong>本站的各项电子服务的所有权和运作权归极目商城所有。用户同意所有注册协议条款并完成注册程序，才能成为本站的正式用户。用户确认：本协议条款是处理双方权利义务的契约，始终有效，法律另有强制性规定或双方另有特别约定的，依其规定。
    </p>
    ...
    <p>
        <strong>1.4</strong>极目商城保留在中华人民共和国大陆地区法施行之法律允许的范围内独自决定拒绝服务、关闭用户账户、清除或编辑内容或取消订单的权利。</p>
    <h5> 第2条 本站服务</h5>

    <p>
        <strong>2.1</strong>极目商城通过互联网依法为用户提供互联网信息等服务，用户在完全同意本协议及本站规定的情况下，方有权使用本站的相关服务。</p>
    ...
    <p>
        <strong>3.1</strong>用户应自行诚信向本站提供注册资料，用户同意其提供的注册资料真实、准确、完整、合法有效，用户注册资料如有变动的，应及时更新其注册资料。如果用户提供的注册资料不合法、不真实、不准确、不详尽的，用户需承担因此引起的相应责任及后果，并且极目商城保留终止用户使用极目商城各项服务的权利。
    </p>
    ...
    <p>
        <strong>3.4</strong><strong>用户同意，极目商城拥有通过邮件、短信电话等形式，向在本站注册、购物用户、收货人发送订单信息、促销活动等告知信息的权利。</strong></p>

    <p>
        <strong>3.5</strong><strong>用户不得将在本站注册获得的账户借给他人使用，否则用户应承担由此产生的全部责任，并与实际使用人承担连带责任。</strong></p>

    <p>
        <strong>3.6</strong><strong>用户同意，极目商城有权使用用户的注册信息、用户名、密码等信息，登陆进入用户的注册账户，进行证据保全，包括但不限于公证、见证等。</strong></p>
    <h5> 第4条 用户依法言行义务</h5>
    ...
    <p>
        本站上的商品价格、数量、是否有货等商品信息随时都有可能发生变动，本站不作特别通知。由于网站上商品信息的数量极其庞大，虽然本站会尽最大努力保证您所浏览商品信息的准确性，但由于众所周知的互联网技术因素等客观原因存在，本站网页显示的信息可能会有一定的滞后性或差错，对此情形您知悉并理解；极目商城欢迎纠错，并会视情况给予纠错者一定的奖励。</p>
    ...
    <p>
        <strong>8.1</strong><strong>用户一旦接受本协议，即表明该用户主动将其在任何时间段在本站发表的任何形式的信息内容（包括但不限于客户评价、客户咨询、各类话题文章等信息内容）的财产性权利等任何可转让的权利，如著作权财产权（包括并不限于：复制权、发行权、出租权、展览权、表演权、放映权、广播权、信息网络传播权、摄制权、改编权、翻译权、汇编权以及应当由著作权人享有的其他可转让权利），全部独家且不可撤销地转让给极目商城所有，用户同意极目商城有权就任何主体侵权而单独提起诉讼。</strong>
    </p>

    <p>
        <strong>8.2</strong><strong>本协议已经构成《中华人民共和国著作权法》第二十五条（条文序号依照2011年版著作权法确定）及相关法律规定的著作财产权等权利转让书面协议，其效力及于用户在极目商城网站上发布的任何受著作权法保护的作品内容，无论该等内容形成于本协议订立前还是本协议订立后。</strong>
    </p>
    ...
    <p>
        <strong>8.4</strong><strong>极目商城是本站的制作者,拥有此网站内容及资源的著作权等合法权利,受国家法律保护,有权不时地对本协议及本站的内容进行修改，并在本站张贴，无须另行通知用户。在法律允许的最大限度范围内，极目商城对本协议及本站内容拥有解释权。</strong>
    </p>

    <p>
        <strong>8.5</strong><strong>除法律另有强制性规定外，未经极目商城明确的特别书面许可,任何单位或个人不得以任何方式非法地全部或部分复制、转载、引用、链接、抓取或以其他方式使用本站的信息内容，否则，极目商城有权追究其法律责任。</strong>
    </p>

    <p>
        <strong>8.6</strong>本站所刊登的资料信息（诸如文字、图表、标识、按钮图标、图像、声音文件片段、数字下载、数据编辑和软件），均是极目商城或其内容提供者的财产，受中国和国际版权法的保护。本站上所有内容的汇编是极目商城的排他财产，受中国和国际版权法的保护。本站上所有软件都是极目商城或其关联公司或其软件供应商的财产，受中国和国际版权法的保护。
    </p>
    ...
    <p>
        <strong>除非另有明确的书面说明,极目商城不对本站的运营及其包含在本网站上的信息、内容、材料、产品（包括软件）或服务作任何形式的、明示或默示的声明或担保（根据中华人民共和国法律另有规定的以外）。</strong>
    </p>

    <p>
        <strong>极目商城不担保本站所包含的或以其它方式通过本站提供给您的全部信息、内容、材料、产品（包括软件）和服务、其服务器或从本站发出的电子信件、信息没有病毒或其他有害成分。</strong></p>

    <p>
        <strong>如因不可抗力或其它本站无法控制的原因使本站销售系统崩溃或无法正常使用导致网上交易无法完成或丢失有关的信息、记录等，极目商城会合理地尽力协助处理善后事宜。</strong></p>
    <h5> 第10条 协议更新及用户关注义务</h5>
    根据国家法律法规变化及网站运营需要，极目商城有权对本协议条款不时地进行修改，修改后的协议一旦被张贴在本站上即生效，并代替原来的协议。用户可随时登陆查阅最新协议；<strong><em>用户有义务不时关注并阅读最新版的协议及网站公告。如用户不同意更新后的协议，可以且应立即停止接受极目商城网站依据本协议提供的服务；如用户继续使用本网站提供的服务的，即视为同意更新后的协议。极目商城建议您在使用本站之前阅读本协议及本站的公告。</em></strong>
    ...
    <p>
        <strong>12.1</strong>极目商城网站所有者是指在政府部门依法许可或备案的极目商城网站经营主体。</p>

    <p>
        <strong>12.2</strong>极目商城尊重用户和消费者的合法权利，本协议及本网站上发布的各类规则、声明等其他内容，均是为了更好的、更加便利的为用户和消费者提供服务。本站欢迎用户和社会各界提出意见和建议，极目商城将虚心接受并适时修改本协议及本站上的各类规则。
    </p>
